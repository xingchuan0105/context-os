import { QueryClient } from '@tanstack/react-query'

export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      staleTime: 30 * 1000, // 30 seconds (reduced from 5 minutes)
      gcTime: 10 * 60 * 1000, // 10 minutes
      retry: 2,
      refetchOnWindowFocus: true, // Enable refetch on focus (was false)
    },
    mutations: {
      retry: 1,
    },
  },
})

export const QUERY_KEYS = {
  knowledgeBases: ['knowledge-bases'] as const,
  knowledgeBase: (id: string) => ['knowledge-bases', id] as const,
  knowledgeBaseShares: (kbId: string) => ['knowledge-base-shares', kbId] as const,
  sharedLink: (token: string) => ['shared-link', token] as const,
  sharedSources: (token: string) => ['shared-link', token, 'sources'] as const,
  sharedNotes: (token: string) => ['shared-link', token, 'notes'] as const,
  sharedSourceIds: (token: string) => ['shared-link', token, 'source-ids'] as const,
  notebooks: ['notebooks'] as const,
  notebook: (id: string) => ['notebooks', id] as const,
  notes: (notebookId?: string) => ['notes', notebookId] as const,
  note: (id: string) => ['notes', id] as const,
  sources: (notebookId?: string) => ['sources', notebookId] as const,
  sourcesInfinite: (notebookId: string) => ['sources', 'infinite', notebookId] as const,
  source: (id: string) => ['sources', id] as const,
  settings: ['settings'] as const,
  adminModelConfigs: ['admin-model-configs'] as const,
  adminModelConfigCapabilities: ['admin-model-config-capabilities'] as const,
  adminModelConfig: (capability: string) => ['admin-model-configs', capability] as const,
  adminLiteLLMModels: ['admin-litellm-models'] as const,
  adminMetricsOverview: (days: number, topUsers: number) => ['admin-metrics', 'overview', days, topUsers] as const,
  adminMetricsLiteLLMUsage: (days: number) => ['admin-metrics', 'litellm-usage', days] as const,
  sourceChatSessions: (sourceId: string) => ['source-chat', sourceId, 'sessions'] as const,
  sourceChatSession: (sourceId: string, sessionId: string) => ['source-chat', sourceId, 'sessions', sessionId] as const,
  notebookChatSessions: (notebookId: string) => ['notebook-chat', notebookId, 'sessions'] as const,
  notebookChatSession: (sessionId: string) => ['notebook-chat', 'sessions', sessionId] as const,
  podcastEpisodes: ['podcasts', 'episodes'] as const,
  podcastEpisode: (episodeId: string) => ['podcasts', 'episodes', episodeId] as const,
  episodeProfiles: ['podcasts', 'episode-profiles'] as const,
  speakerProfiles: ['podcasts', 'speaker-profiles'] as const,
}
